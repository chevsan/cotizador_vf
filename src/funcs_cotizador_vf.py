...
def cleaning_before_split_desarrollo(df,ventana,moneda,grupos):
    '''
    '''
    ### 0) Nulos ###
    ...
    #### 8) Target: construccion final del target ###
    ...
    if moneda == 'dolares':
        ...
        # Target: Los precios en pesos quedan como estan y las filas que tienen precios en dolares se eliminan (de esta forma se obtienen las mejores metricas para un modelo en pesos)
...
def cleaning_before_split_valid(df,ventana,moneda,marc_mod_vers_OK):
    '''
    '''
    ### 0) Nulos ###
    df = df.dropna(subset=['car_year','car_kms','match_marca_a','match_modelo_a','match_v1_a','Subseg_a', 'Seg_a'])
...
def cleaning_before_split_score(df,ventana,moneda,marc_mod_vers_OK):
    '''Con esta funcion lo que haremos unicamente sera levantar warnings para que el usuario este al tanto de que una cotizaciones no
        sera confiable cuando sucedan ciertas cosas como por ejemplo:
         - que haya nulos en alguna de las variables (unico caso que dara error en lugar de warning)
         - que la marca-modelo-version no se haya usado para entrenar al modelo
         - que tenga 11111 en la variables kms
         - que el auto sea 0km o de consecionaria
         '''
    ### 0) Nulos ###
    relevant_features = ['car_year','car_kms','match_marca_a','match_modelo_a','match_v1_a','Subseg_a', 'Seg_a']
    if df[relevant_features].isnull().any().sum() > 0:
        # Aca levantamos un error en lugar de un warning porque el modelo no puede hacer un predict con valores nulos, habria que imputarlo y todavia no decidimos que metodo usar para eso
        raise TypeError("Valores nulos presentes en alguna de las variables que el modelo necesita para cotizar. No es posible cotizar este lead")
    ...
    m9 = df.car_kms == 999
    m10 = df.car_kms == 9999
    m11 = df.car_kms == 99999
    m12 = df.car_kms == 999999
    m13 = df.car_kms == 9999999
    # tratamiento
    all_masks = pd.concat([m1 , m2 , m3 , m4 , m5 , m6 , m7 , m8 , m9 , m10 , m11 , m12 , m13],axis=0)
    if all_masks.sum() > 0:
        #warnings.warn('Uno de los leads/ el lead que queremos cotizar tiene un valor extrano (ej: 11111 o 99999) en la variable km. La cotizacion no sera confiable!')
        print('Uno de los leads/ el lead que queremos cotizar tiene un valor extrano (ej: 11111 o 99999) en la variable km. La cotizacion no sera confiable!')

    ### 4) Dropeamos 0kms y concesionarias
    df['dealer'] = np.where(df['dealer']==True,1,0)
    mask_0km = df.car_kms <= 90
    mask_conces = df.dealer == 1
    all_masks = pd.concat([mask_0km, mask_conces],axis=0)
    if all_masks.sum() > 0:
        #warnings.warn('Uno de los leads/ el lead que queremos cotizar es de concesionaria o tiene menos de 90kms por lo que lo consideramos 0km. El modelo no se entreno con este tipo de casos. La cotizacion no sera confiable!')
        print('Uno de los leads/ el lead que queremos cotizar es de concesionaria o tiene menos de 90kms por lo que lo consideramos 0km. El modelo no se entreno con este tipo de casos. La cotizacion no sera confiable!') 
    ...
    #### 7) Categorías que no nos interesa cotizar ###
    df['marca_modelo_version'] = df['match_marca_a'] + str(' - ') +df['match_modelo_a'] + str(' - ') + df['match_v1_a']
    mask_not_marmodver = df.marca_modelo_version.apply(lambda x: x not in marc_mod_vers_OK)
    if mask_not_marmodver.sum() > 0:
        #warnings.warn('Uno de los leads/ el lead que queremos cotizar es un marca-modelo-version que no se uso para entrenar el modelo. La cotizacion no sera confiable!')
        print('Uno de los leads/ el lead que queremos cotizar es un marca-modelo-version que no se uso para entrenar el modelo. La cotizacion no sera confiable!')   
        print(f'{mask_not_marmodver.sum()} leads son marca-modelo-version que no se usaron para entrenar')
        #print(f'{marc_mod_vers_OK}')

    #### Re-ordenamiento de las cols ### 
    model_features = ['car_year','car_kms','match_marca_a','match_modelo_a','match_v1_a','marca_modelo_version','Subseg_a', 'Seg_a']
...
def cleaning_after_split_train(df_train, vd):
    ...
    for m in modelos:
        for a in años:
            ...
            q1 = data.price_amount.quantile(0.25)
            q3 = data.price_amount.quantile(0.75)
            IQR = q3 - q1
            outl_thresh_superior = q3+3*IQR
            outl_thresh_inferior = q1-3*IQR
            price_thresh_outliers[modelo_año] = (outl_thresh_inferior,outl_thresh_superior)
            filt_mask_sup = data.price_amount>price_thresh_outliers[modelo_año][1]
            filt_mask_inf = data.price_amount<price_thresh_outliers[modelo_año][0]
            data = data[~(filt_mask_sup | filt_mask_inf)]
            df_train = df_train.loc[~(mask1 & mask2),:]
            df_train = pd.concat([df_train,data],0)   
    ...
    model_features = ['car_year','car_kms','match_marca_a','match_modelo_a','match_v1_a','Subseg_a', 'Seg_a', 'price_amount']
    df_train = df_train[model_features].copy()
    df_train['car_year'] = df_train['car_year'].astype('float')
    df_train['car_kms'] = df_train['car_kms'].astype('float')
    
    
    return df_train, thresh_outliers_glob, price_thresh_outliers, kms_thresh_outliers


def cleaning_after_split_test_valid(df, vd, thresh_outliers_glob, price_thresh_outliers, kms_thresh_outliers):
    '''Transform test/ validation/ data to score'''
    ...
    mask = df[cols_to_clean[1]] > thresh_outliers_glob['kms_p995']
    df = df[~mask]
    ...
    # Ultimo retoques
    #   1) Unicamente mantenemos en el df las cols que van a entrenar/entrenaron el modelo
    #   2) Probamos tanto usando year como int y como float y la perfo del modelo dio apenas mejor con year en float
    model_features = ['car_year','car_kms','match_marca_a','match_modelo_a','match_v1_a','Subseg_a', 'Seg_a', 'price_amount']
    df = df[model_features].copy()
...
def cleaning_after_split_score(df, vd, thresh_outliers_glob, price_thresh_outliers, kms_thresh_outliers):

    '''ATENCION: Upgrade pendiente! temporalmente lo vamos a dejar simplemente con un warning y sin ningun tratamiento. Lo ideal seria (ya que vamos a hacer
    las cotizaciones de todas formas para estos casos) hacer por lo menos una imputacion usando dicho threshold para que la cotizacion no use ese outlier.
    '''
    
    cols_to_clean = ['car_kms']
    
    # 1) Outliers globales
    # outliers globales en kms
    mask_outl_glob = df[cols_to_clean[0]] > thresh_outliers_glob['kms_p995']
    if mask_outl_glob.sum() > 0:
        #warnings.warn('Uno de los leads/ el lead que queremos cotizar tiene un valor que sobrepasa el threshold de outliers en la variable kms, revisarlo. La cotizacion no sera confiable!')
        print('Uno de los leads/ el lead que queremos cotizar tiene un valor que sobrepasa el threshold de outliers en la variable kms, revisarlo. La cotizacion no sera confiable!')

    # 2) Outliers por contexto
    # upload the dictionary with the information (learned with the train set!) about the thresholds to cap outliers
    
    # ATENCION: Upgrade pendiente! idem lo que tenemos hacer con los outliers globales.
    #modelos = sorted(list(df.match_modelo_a.unique()))
    #años = sorted(list(df.car_year.unique()))
    #for m in modelos:
    #    for a in años:
...
def evaluate(true, predicted):
    ...
    df_eval = pd.DataFrame({'mae':mae,'mape':mape, 'medape':medape, 'mse':mse, 'rmse':rmse, 'r2_square':r2_square},index=['performance_metrics'])
    return df_eval
